...
import com.tm.cockpits.constants.TmCockpitsConstants;
 ...
 */
@SystemSetup(extension = TmCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(TmCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(TmCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						TmCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/tmcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
